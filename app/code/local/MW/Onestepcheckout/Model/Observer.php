...
if(isset($config_onecheckout['groups']['addfield']['fields']) && isset($config_onecheckout['groups']['config']['fields']) )
{
		...
		if(intval($config['enabled']['value']==1))
		{
			...
			//fix magento ent
			if($addfield['taxvat_show']['value']!=0)
			{
			...
			//{
				...
				if(intval($addfield['zip']['value']) == 1 || intval($addfield['zip']['value']) == 0)
				{	
				...
				if(version_compare(Mage::getVersion(),'1.7.0.0','>=') && (intval($addfield['state']['value'])==1 || intval($addfield['state']['value'])==0))
				{
		...
		try {	
			...
			if(!Mage::helper('onestepcheckout')->onlyProductDownloadable())
			{
	  		...
	  		if($billingmodel->getPostcode() == ".") 
				...
				// save comment and delivery infomation to mw_onestepcheckout
